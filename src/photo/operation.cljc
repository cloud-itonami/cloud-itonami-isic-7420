(ns photo.operation
  "OperationActor -- one photo-studio operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (PhotoOps-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Shoot Delivery Governor (:govern) and
  ...
  One graph run = one photo-studio operation (intake -> advise ->
  ...
  decision to a human operator (studio principal). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/deliver-image-set` ALWAYS reaches this node when the
  governor is clean -- see `photo.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [photo.photoadvisor :as photoadvisor]
...
(defn build
  ...
  `photo.store/Store`).
  opts:
    :advisor      -- a `photo.photoadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (photoadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PhotoOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (photoadvisor/-advise advisor store request)]
            {:proposal p :audit [(photoadvisor/trace request p)]})))

      ;; Shoot Delivery Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
